  ...
  def get_energy(self):
    ...
    if err == 0:
      ...
      if type(payload[0x07]) == int:
        data['energy'] = int(hex(payload[0x07] * 256 + payload[0x06])[2:]) + int(hex(payload[0x05])[2:])/100.0
      else:
        data['energy'] = int(hex(ord(payload[0x07]) * 256 + ord(payload[0x06]))[2:]) + int(hex(ord(payload[0x05]))[2:])/100.0
      return data
